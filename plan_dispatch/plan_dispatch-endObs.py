...
def get_uesd_teles_from_db_by_gu(group_id,unit_id):
    pri_list = []
    date_cur = datetime.datetime.utcnow().strftime("%Y-%m-%d")
    res = pg_act('pd_log_current','select',[['priority'],{'obs_stag':'sent','date_cur':date_cur,'group_id':group_id,'unit_id':unit_id}])
    if res:
        for i in res:
            pri_list.append(i[0])
        pri_list.sort()
                    ...
                    if obj not in send_objs:
                        ...
                        if group_id == 'XL001' and ('GW' in obj_infs['objsour']) and (unit_id not in load_params('./pd_params.json')['obs']['GWAC_init_for_GW']): ###默认 GWAC_init_for_GW 少于 GWAC_init
                            if obj in send_objs:
                                send_objs.remove(obj)
                            break
                        pri_list = get_uesd_teles_from_db_by_gu(group_id,unit_id)
                        obj_pri = obj_infs['priority']
    ...
    while True:
        ...
        if msg:
            ...
            if t_wasm:
                ...
                t_wasm = None
            mark_wait = 0
            t_cn1 = Mythread_get_result(target=check_new_new_main,args=('XL001',))
            t_cn1.setDaemon(True)
            t_cn1.start()
            t_cn2 = Mythread_get_result(target=check_new_new_main,args=('XL002',))
            t_cn2.setDaemon(True)
            t_cn2.start()
            t_cn3 = Mythread_get_result(target=check_new_new_main,args=('XL003',))
